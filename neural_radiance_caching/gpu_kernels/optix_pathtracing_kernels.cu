        ...
        uint32_t trainDataIndex;
        {
            ...
            if constexpr (useNRC) {
                ...
                if (isTrainingPath) {
                    ...
                    atomicAdd(plp.s->numTrainingData[bufIdx], 1u);
                    trainDataIndex = linearIndex;
...
CUDA_DEVICE_FUNCTION CUDA_INLINE void pathTrace_closestHit_generic() {
    ...
    if constexpr (useNRC) {
        ...
        // EN: Allocate a training data entry.
        if (rwPayload->isTrainingPath && !rwPayload->trainingSuffixEndsWithCache) {
            // uint32_t trainDataIndex = 512*512 + atomicAdd(plp.s->numTrainingData[bufIdx], 1u);
            uint32_t trainDataIndex = 512*512 + atomicAdd(plp.s->numIntermediateTrainingData[bufIdx], 1u);
